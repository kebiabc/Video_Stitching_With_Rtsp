...
public:
    ...
    cv::UMat getFinalXMap(size_t img_idx) const {
        return final_xmap_vector_[img_idx];
    }

    cv::UMat getFinalYMap(size_t img_idx) const {
        return final_ymap_vector_[img_idx];
    }

    const std::vector<cv::Rect>& getRoiVector() const {
        return roi_vect_;
    }
